...
asIScriptEngine* AngelScriptUtil::getEngine() {
	if (s_engine == NULL)
	{
        ...
        r = s_engine->RegisterObjectMethod("Renderer", "void fillCircle(float x, float y, float radius)", asMETHODPR(Renderer,fillCircle, (float, float, float) const, void), asCALL_THISCALL); assert( r >= 0 );
		r = s_engine->RegisterObjectMethod("Renderer", "void fillCircle(float x, float y, float radius, int points)", asMETHODPR(Renderer,fillCircle, (float, float, float, int) const, void), asCALL_THISCALL); assert( r >= 0 );
