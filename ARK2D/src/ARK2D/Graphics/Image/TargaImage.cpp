...
// -1: something else...
//  0: success
//  1: could not open image for reading
//  2: unsupported image type
//  3: unsupported color depth, see m_bytesPerPixel
int TargaImage::load(const string& filename) {
    ifstream fileIn(filename.c_str(), std::ios::binary);

    if (!fileIn.good())
    {
        //std::cerr << "Could not open the targa image file for reading" << std::endl;
    	//std::string errmsg = "Could not open the targa image file for reading";
		//MessageBox(NULL, errmsg.c_str(), NULL, MB_OK | MB_ICONEXCLAMATION);
		return 1;
    }
    ...
    if (!isImageTypeSupported(m_header))
    {
    	...
    	//std::string errmsg = "This is not a supported image type";
		//MessageBox(NULL, errmsg.c_str(), NULL, MB_OK | MB_ICONEXCLAMATION);
		return 2;
    }
    ...
    {
        bool suc = loadUncompressedTarga(fileIn);
        result = (suc==true)?0:-1;
    }
    else
    {
        bool suc = loadCompressedTarga(fileIn);
        result = (suc==true)?0:-1;
    }
