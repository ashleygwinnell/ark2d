...
namespace ARK {
	namespace Core {
		namespace Graphics {
			...
			Image::Image():
				ARK::Core::Resource(),
				ARK::Core::SceneGraph::SceneNode("image",SceneNode::TYPE_IMAGE),
				m_data(NULL),
				m_dataLength(0),
				m_resourceType(0),
				filename(""),
			...
			Image::Image(void* data, unsigned int resourceType, string file):
				ARK::Core::Resource(),
				ARK::Core::SceneGraph::SceneNode(file,SceneNode::TYPE_IMAGE),
				m_data(data),
				m_dataLength(0),
				m_resourceType(resourceType),
				filename(file),
			...
			Image::Image(const std::string& fname):
				ARK::Core::Resource(),
				ARK::Core::SceneGraph::SceneNode(fname,SceneNode::TYPE_IMAGE),
				m_data(NULL),
				m_resourceType(0),
				filename(fname),
				m_bounds(0,0,0),
			...
			Image::Image(FBO* fbo):
				ARK::Core::Resource(),
				ARK::Core::SceneGraph::SceneNode("fbo_texture",SceneNode::TYPE_IMAGE),
				m_data(NULL),
				m_resourceType(0),
				filename(""),
				m_bounds(0,0,0),
				...
				m_dirty(false)
			{
				// textures are a bit botched here. reloading fbos on android may not work, etc.

				texture_temp = fbo->getTextureId();
				m_texture = new Texture(texture_temp, this);

				m_Width = fbo->getWidth();
				m_Height = fbo->getHeight();
				m_originalWidth = m_Width;
				m_originalHeight = m_Height;
			...
			ARK::Core::Geometry::Cube* Image::getBounds()
			{
				if (m_dirty) { clean(); }
				return &m_bounds;
			}
			...
			void Image::drawSubImage(int x1, int y1, int x2, int y2, int w1, int h1, int w2, int h2) {
				...
				g->matrixMode(MatrixStack::TYPE_MODEL);
				g->pushMatrix();
				g->translate(x2, y2);
