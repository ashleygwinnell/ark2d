...
TiledMap::TiledMap(const string& file, void* data):
	...
	m_tileHeight(0)
{
	m_xmldocument.Parse((const char*)m_data, 0, TIXML_ENCODING_UTF8);
	parse();
}
...
void TiledMap::parse() {
	...
	for (tileset_e = mapelement->FirstChildElement("tileset"); tileset_e;
		...
		string path = StringUtil::pathToFile(m_file); // this piece makes sure the tileset is in the same folder as the map.
		if (path != m_file) {
			src.insert(0, path);
		}
		...
		if (m_data == NULL) {
			...
			if (!ts_loadOkay) {
				...
				ErrorDialog::createAndShow("Could not get tiled map because reading from data buffer and not on android. TODO: this.");
				exit(0);
			#endif
...
void TiledMap::parseTileset(unsigned int& firstgid, string& src, TiXmlDocument* tileset_d) {
	...
	if (!hasImageMask) {
		...
		img_obj = ARK::Resource::get(image_src, false)->asImage(); // TODO: image mask in resource::get();
	}
